...
export const elementMethodsMixins = {
  ...
  methods: {
    ...
    axiosCommon(reqData, obj, fn) {
      ...
      const url = obj.url.indexOf('http') > -1 ? obj.url : this.settingConfig.dataUrl + obj.url
      axios
        .post(url, reqData)
        .then(res => {
    ...
    // 特殊图表数据处理
    getchartsList(obj) {
      const title = obj.config.contentAreaConfig.title
      if (title === '树形表格') {
        obj.sftsqk(true) // 确认未特殊情况，拦截默认数据请求处理
        const data = [{
          id: 1,
    ...
    kjxmcssj(obj) {
      ...
      if (csmoduleIdArr.indexOf(obj.config.moduleId) > -1) {
        // 项目成果管理
        obj.sftsqk(true)
        const keyArr = obj.config.contentAreaConfig.keyArr
        ...
        obj.sftsqk(true)
        const data = [{
          gg: '解放军军机进入台湾西南空域解放军军机进入台湾西南空域解放军军机进入台湾西南空域'
        },
        {
          gg: '青岛涉冷链样本检出51份阳性'
        },
