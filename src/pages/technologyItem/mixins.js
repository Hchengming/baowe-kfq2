...
export const elementMethodsMixins = {
  ...
  mounted() {
    serviceAxios
      .post('http://23.36.25.161:8086/science/declare/page', {
        pageCurrent: 1,
        pageSize: 10
      })
      .then(res => {
  ...
  methods: {
    ...
    getchartsList(obj) {
      ...
      if (obj.config.moduleId === 'be548b710e60489d8b62209d4952c7de') {
        ...
        this.axiosCommon({}, obj, resData => {
          const datas = resData.kfqdjPOS.concat(resData.kfqpjflPOS)
          obj.tsqkData(datas)
        })
        ...
        this.axiosCommon(reqData, obj, resData => {
          const datas = resData.kfqflqkPOS.kfqdjPOS.concat(
        ...
        this.axiosCommon(reqData, obj, resData => {
          const datas = resData.kfqydgcqkPOS
          obj.tsqkData(datas)
        })
    ...
    // 科技项目管理系统--测试数据导入
    kjxmcssj(obj) {
      if (obj.config.moduleId === 'c0b9411a32474571a5bf14cc1337a4bc') {
        obj.sftsqk(true)
        const data = [
          {
            modelName: '申报',
            menuCode: 'shenbaomingxibiao1',
            count: 10
          },
          {
            modelName: '立项',
            menuCode: 'LIXIANGMINGXIBIAO',
            count: 10
          },
          {
            modelName: '实施中',
            menuCode: 'SHISHIMINGXIBIAO',
            count: 10
          },
          {
            modelName: '结题',
            menuCode: 'JIETIMINGXIBIAO',
            count: 10
          }
        ]
        obj.tsqkData(data)
