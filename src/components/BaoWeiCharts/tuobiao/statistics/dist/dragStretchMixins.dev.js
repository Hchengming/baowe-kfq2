...
var _default = {
  data: function data() {
    return {
      ...
      cursor: 'defalut' // shubiaoMove: '' // 鼠标拖拽状态样式
  ...
  methods: {
    // 内容区域宽高获取
    getMainStyle: function getMainStyle() {
    ...
    mousedown_tz: function mousedown_tz(e) {
      ...
      var _this = this; //  设置定时器，鼠标按下一秒后启动拖拽事件


      var timer = null;
      var mousedownTzZx = null;
      this.cursor = '';
      clearTimeout(timer); // console.log(this.settingForm.isDrafting)

      timer = setTimeout(function () {
        // 权限控制
        if (_this2.isAdmin || _this2.settingForm.isDrafting === true) {
          mousedownTzZx(e);
          _this2.cursor = 'move';
        }
      }, 200);
      var drag = this.$refs['statisticsWrap'];

      mousedownTzZx = function mousedownTzZx(e) {
      ...
      document.onmouseup = function () {
        ...
        this.onmouseup = null;
        clearTimeout(timer); // console.log('onmouseup')

        if (_this.cursor === 'move' && _this.isAdmin) {
          _this.TZLSKeep();
        }

        _this.cursor = ''; // 修复低版本ie bug
    ...
    // 左侧缩放
    mousedown_left_ls: function mousedown_left_ls(e) {
      this.mousedown_ls(e, 'left');
    },
    // 右侧缩放
    mousedown_right_ls: function mousedown_right_ls(e) {
      this.mousedown_ls(e, 'right');
    },
    // 模块拉伸事件
    mousedown_ls: function mousedown_ls(event, type) {
      // console.log('mousedown')
      var _this = this;

      this.stopPropagation(event);
      this.stopDefault(event); // let event = e || windeow.event;  //兼容IE浏览器
      ...
      var modelStyleClone = JSON.parse(JSON.stringify(_this.modelStyle));
      var width = modelStyleClone.width;
      var height = modelStyleClone.height;
      var left = modelStyleClone.left;

      document.onmousemove = function (event) {
        // eslint-disable-next-line no-redeclare
        var event = event || window.event;
        var moveX = event.clientX - diffX;
        var moveY = event.clientY - diffY; // 模块最小宽高限制
    ...
    TZLSKeep: function TZLSKeep() {
      this.$emit('updateMoule', this.settingForm, this.statisticsAll.moduleId, function () {// this.$message({
        //     type: 'success',
        //     message: '模块布局保存成功!'
        // });
      }, this.whereForm);
