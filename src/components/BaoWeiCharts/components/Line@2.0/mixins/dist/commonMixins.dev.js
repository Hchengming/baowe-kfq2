...
var _default = {
  data: function data() {
    return {
      ...
      colorArr: ['#19D4AE', '#5AB1EF', '#FF7993', '#FFB980', '#0071B6', '#C4B4E4', '#D87A80', '#9CBBFF', '#D9D0C7', '#D49EA2', '#439586', '#63C2FF', '#FA7189', '#486F86', '#D7C6FA', '#39767E', '#7BA3A8', '#E8A51A', '#8BFFFC', '#172421'] // 默认背景颜色集合
  ...
  methods: {
    // 主题切换
    themeConfjg: function themeConfjg() {
    ...
    // echart初始化
    echartsInit: function echartsInit() {
      if (!this.data || this.data.length === 0) return; // e-charts图表清空，然后重新加载
      ...
      var myChart = this.$echarts.init(this.$refs['myCharts']);
      if (!myChart) return;
      var options = {}; // 主题切换照成字体颜色修改

      this.themeConfjg(); // 1、图表字段配置数据获取

      this.setChartColumns(); // 2、图表边距位置设定

      this.setGridsetGrid(options); // 3、柱状图/条形图/折线图/雷达图 x轴、y轴公共配置
      ...
      this.setLegend(options); // 5、series图表显示配置

      this.setSeries(options); // 6、悬浮框 tooltip 配置

      this.setTooltip(options); // 7、图表其他设置

      this.otherSetting(options); // 8、图形背景颜色设置

      this.setColor(options); // 9、鼠标事件
    ...
    // 7、图表其他设置
    otherSetting: function otherSetting(options) {
      // 01、饼图、环图显示数据设置
      this.pieLabelSetting(options);
    },
    // 图表点击事件
    chartClick: function chartClick(params, options, myChart) {
      // 柱状图、条形图点击状态事件
    ...
    setChartColumns: function setChartColumns() {
      var _this4 = this;
      ...
      this.chartColumn.forEach(function (item) {
        ...
        if (item.ischartsShow && !item.ischartsTitle) {
          ...
          item.title = item.explain + dw; // item.title = item.explain

          _this4.chartColumns.push(item);
        } // 02 查找标题字段


        if (item.ischartsTitle) {
          _this4.titleKey = item.key;
        }
    ...
    // 图表边距位置设置
    setGridsetGrid: function setGridsetGrid(options) {
      var _this5 = this;

      var gridLeft = 10; // 柱状图/条形图/折线图/雷达图 配置

      if (['histogram', 'bar', 'line', 'radar'].indexOf(this.chartType) > -1) {
        var titleMaxlength = 0;
        var vauleMaxlength = 0; // 获取y轴为标题轴，数值轴宽度

        this.data.forEach(function (items) {
          if (items[_this5.titleKey] && items[_this5.titleKey].length > titleMaxlength) {
            titleMaxlength = items[_this5.titleKey].length;
          }

          _this5.chartColumns.forEach(function (item) {
            if (item.key !== _this5.titleKey && items[item.key] && items[item.key].toString().length > vauleMaxlength) {
        ...
        if (['histogram', 'line', 'radar'].indexOf(this.chartType) > -1) {
          var num = Math.floor(vauleMaxlength / 3);
          gridLeft = vauleMaxlength * 5 + num * 5 + 15; // console.log(vauleMaxlength, num, 'bar')
        } else if (['bar'].indexOf(this.chartType) > -1) {
          gridLeft = titleMaxlength * 11 + 20; // console.log(titleMaxlength, 'histogram', gridLeft)
        }
      }
    ...
    // 5、series图表显示配置
    setSeries: function setSeries(options) {
      options.series = [];
      ...
      switch (this.chartType) {
        ...
        case 'radar':
          this.setRadarSeries(options); // 雷达图雷达背景布局
    ...
    // 图表高度配置
    chartsHeight: function chartsHeight() {
      var chartsHeight = this.height; // console.log(['pie', 'ring'].indexOf(this.chartType))
