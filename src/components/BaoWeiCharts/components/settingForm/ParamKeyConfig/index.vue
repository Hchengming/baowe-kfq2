<template>
  <div class="fieldset-list">
    <fieldset class="param-config-setting">
      ...
      <el-button v-if="componentType!=='iframe'" size="small" @click="getparamConfig">参数获取</el-button>
      <br>
      <ul class="zdpz_list param-config-list">
        ...
        <li v-for="(item,index) in form.paramConfig" :key="index" class="zdpz_list_content">
          <span>
            <el-input v-model="item.paramKey" size="mini" placeholder="参数名" />
          </span>
          <span>
            <el-input v-model="item.description" size="mini" placeholder="释义" />
          </span>
          <span>
            <el-input
          ...
          <span>
            <el-input v-model="item.dataType" size="mini" placeholder="数据类型" />
          </span>
          <span>
    ...
    <!-- 树形弹窗 -->
    <tree-model ref="treeModel" :tree-data="treeData" @elTreeSubmit="elTreeSubmit" />
  </div>
</template>
<script>
