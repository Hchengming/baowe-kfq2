...
var _default = {
  ...
  mounted: function mounted() {
    this.getMainStyle();
    this.settingForm = JSON.parse(JSON.stringify(this.config));
    this.listData = this.dataSource.map(function (value) {
      return value;
    });
    this.setDemos();
  },
  methods: {
    ...
    mousedown_tz: function mousedown_tz(e) {
      ...
      timer = setTimeout(function () {
        // 权限控制
        if (_this2.settingForm.isDrafting === true) {
          mousedownTzZx(e);
          _this2.cursor = 'move';
        }
      }, 200);
      var drag = this.$refs['listWrap'];
