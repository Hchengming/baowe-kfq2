<template>
  <!-- 表单项数据配置模块 -->
  <div>
    <el-dialog ref="screenFormSettings"
      ...
      <el-form ref="form">
        ...
        <div v-if="dataType=='custom'">
          <el-row v-for="(item,index) in settingData"
                  :key="index"
                  type="flex"
                  class="row-bg">
            <el-col :span="10">
            ...
            <el-col :span="4">
              <div class="rightIcon">
                ...
                <i :class="['iconfont','iconxiayi',{'disabled':settingData.length-1==index}]"
                   @click="sortNext(item,index,settingData.length-1==index)" />
                <i class="el-icon-delete"
                   @click="screenDelete(item,index)" />
              </div>
            </el-col>
          </el-row>
        </div>
