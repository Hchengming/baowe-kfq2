...
var _commonWhere = _interopRequireDefault(require("./commonWhere.json"));
...
var _default = {
    ...
    return {
      ...
      // },
      {
        ...
        children: [{
          ...
          isHide: function isHide(form) {
            return ['radio', 'checkbox', 'select'].indexOf(form.type) === -1;
          },
          click: function click(form, fatherIndex, item) {
            _this.itemDataChange(form, fatherIndex, item); // console.log('配置数据/接口')
          ...
          isHide: function isHide(form, index) {
            var offon = true;

            if (['date'].indexOf(form.type) > -1) {
              var kvp = _this.form.filterConfig.screenData[index - 1];
  ...
  methods: {
    // 默认参数传递
    getmrParams: function getmrParams() {
      var _this2 = this;

      var screenData = this.form.filterConfig.screenData;
      var paramConfig = this.form.paramConfig;
      if (!this.form.paramConfig || this.form.paramConfig.length === 0) return;
      paramConfig.forEach(function (item) {
        var offon = true;

        if (item.isUse) {
          screenData.forEach(function (x) {
          ...
          if (offon) {
            ...
            _this2.tableCloums.forEach(function (xx) {
              if (xx.children) {
                xx.children.forEach(function (yy) {
                  obj[yy.key] = null;
                });
              } else {
                obj[xx.key] = null;
    ...
    // 是否结束时间变化事件
    isOverTimeChange: function isOverTimeChange(items, item) {
      if (items[item.key] === '1') {
        items.label = '';
      }
    },
    // 标签名变化事件--获取自适应右侧标签宽度
    labelChange: function labelChange(items, index, item) {
      // let length=items[item.key]
      if (item.key === 'label') {
        var length = items.label.length;
    ...
    // 区县，单选项选中事件
    checkCountryRadio: function checkCountryRadio(item) {
      _commonWhere["default"].forEach(function (obj) {
    ...
    onSubmit: function onSubmit(fn) {
      var offon = false; // 筛选表单配置数据校验

      var conditionAreaConfig = this.form.filterConfig.screenData;
      conditionAreaConfig.forEach(function (item) {
      ...
      var offon2 = false;
      this.btnSettingData.forEach(function (item) {
      ...
      if (offon || offon2) {
        ...
        if (offon) {
          err1 = '表单配置写完整(字段名不能为空，单选、多选、下拉框配置数据不能为空);';
        }
    ...
    // 表单提交事件
    // 数据配置框获取焦点事件
    itemDataChange: function itemDataChange(form, fatherIndex, item) {
      this.otherNowIndex = fatherIndex;
      this.$refs.settingData.show(form[item.key]);
    },
    // 数据配置确认事件
    itemDataConfig: function itemDataConfig(val) {
      // console.log(val)
