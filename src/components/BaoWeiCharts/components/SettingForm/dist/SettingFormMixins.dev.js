...
var otherMixins = {
  ...
  methods: {
    ...
    show: function show(obj) {
      ...
      this.keyChooseAllShow(); // console.log(this.form)
    },
    // 配置数据旧版本兼容处理
      ...
      if (this.form.moduleType === '0') {
        this.form.keyArr.forEach(function (item) {
          if (item.isShow === false) {
            _this4.listKeyAll = false;
          }

          if (item.ischartsShow === false) {
            _this4.chartsKeyAll = false;
          }
        });
      }
    },
    ...
    urlNameChange: function urlNameChange(val) {
      ...
      this.itemApiData.forEach(function (item) {
        if (item.apeName === val) {
          _this6.form.url = item.aaaRequestUrl;
          _this6.form.options = item.method;
        }
      });
    },
    // 接口路径变化事件
    urlChange: function urlChange(val) {
      ...
      this.itemApiData.forEach(function (item) {
        if (item.aaaRequestUrl === val) {
          ...
          _this7.form.urlName = item.apeName;
        }
      });
    }
  }
